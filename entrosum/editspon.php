...
include('sessions_admin.php');
	...
	{
		$id = $_GET['edit'];
		$res= mysqli_query($db,"SELECT * FROM fusion WHERE id='$id' and type='sponsors'");
...
function GetImageExtension($imagetype)
 {
   ...
   switch($imagetype)
   {
	...
	{
    $target = "../entrosum/img/sponsors/".basename($_FILES["newfileimg"]["name"]);
...
{
   $sql = "UPDATE fusion SET imgpath='$newfileimg',type='sponsors',link='$newLink',name='$newName',contact='$newContact',email='$newEmail' WHERE id='$id'";
...
else // no image uploaded
{
   // save data, but no change the image column in MYSQL, so it will stay the same value
    $sql = "UPDATE fusion SET type='sponsors',link='$newLink',name='$newName',contact='$newContact',email='$newEmail' WHERE id='$id'";
}
...
<body>
  <div class="container-fluid">
    ...
    <div class="row" style="height:100vh; width: 100vw;">
      ...
      <div class="col-md-10">
      <div class="container">
        <form  action="editspon.php" enctype="multipart/form-data" method="post">


            <div class="form-group">
              <?php   $imageURL = '../entrosum/img/sponsors/'.$row['imgpath'];?>
